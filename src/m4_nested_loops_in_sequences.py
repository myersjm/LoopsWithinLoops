...
def multiply_numbers(sequence_of_lists):
    ...
    for k in range(len(sequence_of_lists)):
        for j in range(len(sequence_of_lists[k])):
            examinedList = sequence_of_lists[k]
            examinedList[j] = examinedList[j] * (k + 1)
...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = -7
    answer = sum_numbers(([-5, 5, 1], [-20, 0, 1], [10, 0, 1]))
    print('Expected and actual are:', expected, answer)
...
def sum_numbers(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            examinedList = seq_seq[k]
            total = total + examinedList[j]
    return total
...
def print_characters(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        for j in range(len(sequence_of_strings[k])):
            examinedString = sequence_of_strings[k]
            print(examinedString[j])
